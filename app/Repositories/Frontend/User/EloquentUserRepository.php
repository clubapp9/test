...
class EloquentUserRepository implements UserContract
{
    ...
    public function updateProfile($input)
    {
        ...
        if(isset($input['first_name']))  $user->first_name = $input['first_name'];

       if(isset($input['last_name'])) $user->last_name = $input['last_name'];

        if(isset($input['title'])) $user->title = $input['title'];

        if(isset($input['organization'])) $user->organization = $input['organization'];

        if(isset($input['phone'])) $user->phone = $input['phone'];

        if(isset($input['speakers_bureau'])) {
            $user->speakers_bureau = $input['speakers_bureau'];
        }else {
            $user->speakers_bureau = false;
        }

        if(isset($input['short_bio'])) $user->short_bio = $input['short_bio'];
